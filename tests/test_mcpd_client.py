...
class TestMcpdClient:
    @pytest.fixture
    def client(self):
        return McpdClient(api_endpoint="http://localhost:8090")

    @pytest.fixture
    def client_with_auth(self):
        return McpdClient(api_endpoint="http://localhost:8090", api_key="test-key")
    ...
    def test_init_with_auth(self, client_with_auth):
        assert client_with_auth.endpoint == "http://localhost:8090"
        assert client_with_auth.api_key == "test-key"
        assert "Authorization" in client_with_auth.session.headers
        assert client_with_auth.session.headers["Authorization"] == "Bearer test-key"
    ...
    def test_servers_success(self, mock_get, client):
        mock_response = Mock()
        ...
        mock_get.assert_called_once_with("http://localhost:8090/api/v1/servers", timeout=5)
    ...
    def test_perform_call_success(self, mock_post, client):
        ...
        mock_post.assert_called_once_with(
            "http://localhost:8090/api/v1/servers/test_server/tools/test_tool",
            json={"param": "value"},
            timeout=30
        )
